...
def aspp_block(
    ...
    conv3_3_1 = BatchNormalization(
            axis=bn_axis,
            name='bn_3_3_1_%s' % output_stride)(conv3_3_1)
    ...
    conv3_3_2 = BatchNormalization(
            axis=bn_axis,
            name='bn_3_3_2_%s' % output_stride)(conv3_3_2)
    ...
    conv3_3_3 = BatchNormalization(
            axis=bn_axis,
            name='bn_3_3_3_%s' % output_stride)(conv3_3_3)
...
def interp_block(
    ...
    global_feat = AveragePooling2D(
            ...
            strides=strides,
            padding='same',
            name='pool_level_%s_%s' % (level, output_stride))(x)
    global_feat = _conv(
            filters=num_filters,
            kernel_size=(1, 1),
            padding='same',
    ...
    global_feat = Lambda(
            Interp,
            arguments={'shape': feature_map_shape})(global_feat)
...
def pyramid_pooling_module(
    ...
    y = BatchNormalization(
            axis=bn_axis,
            name='bn_pyramid_out_%s' % output_stride)(y)
    y = Activation('relu')(y)
...
def PSPNet50(
    ...
    if upsample_type == 'duc':
        ...
        out = _conv(
                ...
                kernel_size=(1, 1),
                padding='same',
                block='out_duc_%s' % output_stride)(x)
        ...
        out = BilinearUpSampling2D(
                (n_labels, input_shape[0], input_shape[1]),
