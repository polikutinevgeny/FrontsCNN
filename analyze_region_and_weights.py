...
with xr.open_dataset(file, cache=False, chunks={'time': 100}) as ds:
    for v in varnames:
        d[v] = (float(ds[v].sel(time=dates).min().compute().values), float(ds[v].max().compute().values))
        # h, bins = da.histogram(ds[v].sel(time=dates), bins=100, range=(d[v][0], d[v][1]))
        # plt.hist(bins[:-1], weights=h, bins=)
        plt.hist(ds[v].sel(time=dates).data.ravel(), bins=100)
        plt.title(v)
        plt.show()
print(d)
